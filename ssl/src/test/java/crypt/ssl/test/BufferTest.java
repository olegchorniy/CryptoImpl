...
public class BufferTest {
    ...
    public void basicTest() {
        ...
        assertEquals(bytes(0x71, 0x72), buffer.getBytes(2));
        assertEquals(1, buffer.available());

        buffer.putBytes(bytes(0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67));

        assertEquals(8, buffer.available());
        assertEquals(bytes(0x73, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67), buffer.peekBytes());
        assertEquals(8, buffer.available());
        assertEquals(bytes(0x73, 0x61, 0x62, 0x63), buffer.getBytes(4));
        assertEquals(4, buffer.available());

        buffer.skip(2);

        assertEquals(2, buffer.available());
        assertEquals(bytes(0x66, 0x67), buffer.getBytes());
    }
